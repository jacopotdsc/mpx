...
def set_base_height_target(state, height_target: float):
...
def build_hud(command_vec, target_vec, actual_vec, help_text):
    """HUD with actual, smoothed command and target command side by side.

    actual_vec:
      TITA -> [local vx, gyro wz]
      quadruped -> [local vx, local vy, gyro wz]

    command_vec <- state.info["command"]
    target_vec  <- state.info["target_command"]
    """
    ...
    for actual, command, target in zip(actual_vec, command_vec, target_vec):
        value_lines.append(
            f"{float(actual):+7.2f}  "
...
def main(
    ...
    def key_callback(keycode):
        ...
        if keycode == KEY_PAGE_UP:
            ...
            print(f"[height] target = {height_target:.2f} m")
            return
    ...
    state = set_base_height_target(state, height_target)
